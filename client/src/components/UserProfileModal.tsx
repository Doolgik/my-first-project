...
export default function UserProfileModal({ userId, onClose, onOpenConversation }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-end justify-center bg-black/60 sm:items-center" onClick={onClose}>
      <div className="w-full max-w-md animate-fade-in rounded-t-2xl bg-slate-900 p-6 pb-safe shadow-2xl sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        {!user ? (
          ...
          <>
            ...
            <div className="mt-6 grid grid-cols-3 gap-2">
              <button onClick={openChat} disabled={busy} className="flex flex-col items-center gap-1 rounded-xl bg-slate-800 py-3 text-brand-400 transition hover:bg-slate-700">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>
                <span className="text-xs">Message</span>
              </button>
              <button onClick={() => call('audio')} className="flex flex-col items-center gap-1 rounded-xl bg-slate-800 py-3 text-emerald-400 transition hover:bg-slate-700">
              ...
              <button onClick={() => call('video')} className="flex flex-col items-center gap-1 rounded-xl bg-slate-800 py-3 text-sky-400 transition hover:bg-slate-700">
            ...
            <button
              ...
              className={`mt-3 w-full rounded-xl px-4 py-3 font-medium transition ${
                contact ? 'border border-slate-700 bg-slate-800 text-slate-300 hover:bg-slate-700' : 'bg-brand-600 text-white hover:bg-brand-500'
              }`}
            >
